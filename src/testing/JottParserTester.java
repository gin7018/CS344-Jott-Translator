...
public class JottParserTester {
    ...
    private void createTestCases(){
      ...
      //need to fix ptype return//  testCases.add(new TestCase("missing return", "missingReturn.jott", false ));
       //not checking function parama types quick fix in FunctionParameters// testCases.add(new TestCase("Void not valid param type (error)", "voidParam.jott", true ));
       // testCases.add(new TestCase("function not defined", "funcNotDefined.jott", false ));
        //ptpye issue again probaly woth storing token so we know we error might be// testCases.add(new TestCase("mismatch return type", "mismatchedReturn.jott", false ));
      //same ptype issue//  testCases.add(new TestCase("function call param type not matching", "funcCallParamInvalid.jott", false ));
        //error should be good//testCases.add(new TestCase("single expression program (error)", "singleExpr.jott", true ));
       //ptype// testCases.add(new TestCase("valid while loop", "validLoop.jott", false ));
    //ptype//    testCases.add(new TestCase("missing main", "missingMain.jott", false ));
   //p//     testCases.add(new TestCase("main must be integer", "mainReturnNotInt.jott", false ));
      //p and while //  testCases.add(new TestCase("i_expr relop d_expr function return", "funcReturnInExpr.jott", false ));
      //error should be good//  testCases.add(new TestCase("invalid asmt stmt (error)", "invalidAsmtStmt.jott", true ));
       //f param is not checking for commas just removes them// testCases.add(new TestCase("missing comma in func_def_params (error)", "missingCommaParams.jott", true ));
